...
fn p2_validate(passport: &Vec<Vec<&str>>) -> u32 {
    ...
    // let pass_keys = passport.iter().map(|x| x[0]).collect::<Vec<&str>>();
...
fn p1_run(raw: &str) -> u8 {
    let parsed = prepare_data(raw);
...
fn p2_run(raw: &str) -> u32 {
    let parsed = p2_prepare_data(raw);
...
iyr:2017 ecl:blu byr:1942 hcl:#733820 eyr:2023 hgt:151cm pid:289923625";
    ...
    // let count = p1_run(raw);
    let count = p2_run(raw);
    println!("{}", count);
...
pid:3556412378 byr:2007";
    ...
    fn p2_valid_counter_valid() {
        ...
        let raw = "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
